...
/**
  ...
  */
class phNhwaDeliveryJob(args: Map[String, String]) extends pActionTrait with phSparkCommonFuncTrait {
    ...
    override def perform(pr: pActionArgs): pActionArgs = {
        ...
        val deliveryDF = {
            ...
            val df_match_hospital = driver.csv2RDD(max_path_obj.p_matchFilePath + args("hospital_match_file"))
            val df_match_nhwa = driver.csv2RDD(max_path_obj.p_matchFilePath + args("nhwa_match_file")).select("药品名称_标准","商品名_标准","药品规格_标准","剂型_标准","生产企业_标准","min1_标准","Pack_ID","商品名+SKU","毫克数").distinct()
            val df_match_acc = driver.csv2RDD(max_path_obj.p_matchFilePath + args("acc_match_file"))
            val df_match_area = driver.csv2RDD(max_path_obj.p_matchFilePath + args("area_match_file"))
            val df_match_market = driver.csv2RDD(max_path_obj.p_matchFilePath + args("market_match_file"))
