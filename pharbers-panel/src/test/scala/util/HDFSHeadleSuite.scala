...
class HDFSHeadleSuite extends FunSuite with phPanelFilePath {
    ...
    test("test create file") {
        ...
        hdfs.create(new Path("/user/jeorch/test2.txt"))
    }
    ...
    test("spark write csv in hdfs") {
        val conf = new Configuration()
        conf.set("fs.defaultFS", "hdfs://192.168.100.174:54188/user/jeorch")
        conf.set("hadoop.job.ugi", "hadoop")
        val file = new Path("/user/jeorch/test2.txt")
        val hdfs = FileSystem.get(conf)
        val a = hdfs.create(file)

        val writer = new PrintWriter(a)
